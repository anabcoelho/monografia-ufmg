...
### [REVISÃO] Verificação da Estrutura de dados e tratamento de valores ausentes
...
O fator 1.5 no critério do IQR foi proposto por @Tukey1977 como um compromisso empírico entre robustez e sensibilidade, sendo amplamente adotado na literatura por sua boa performance em distribuições assimétricas e com caudas longas, como é típico em dados financeiros.
...
### [REVISÃO] Codificação de variáveis categóricas
...
#### [REVISÃO] Divisão entre treino e teste
...
As variáveis (a) Renda Anual, (c) Prazo do Empréstimo e (d) CIBIL Score mostram uma distribuição quase uniforme. Isso sugere que a base de dados é bem balanceada em termos de perfil de clientes. Não há uma concentração excessiva em pessoas ricas ou pobres, nem em scores de crédito específicos. Para a variável de Prazo (c), podemos ver que gráfico apresenta "picos" isolados, o que é esperado para prazos de empréstimo, que geralmente são fixados em números inteiros de anos.

As variáveis (e) Ativos Residenciais, (f) Ativos Comerciais, (g) Bens de Luxo e (h) Ativos Bancários apresentam uma clara assimetria à direita. Indicando que a  maioria dos indivíduos possui ativos de valor menor, enquanto uma minoria detém grandes fortunas.
...
A (b) Distribuição do valor do empréstimo é interessante: ela se mantém constante até um certo patamar (em torno de $2 \cdot 10^7$) e depois sofre uma queda acentuada.Análise: Isso pode indicar que o banco tem um limite de crédito comum para a maioria dos clientes, e empréstimos acima desse valor são exceções ou exigem critérios muito mais rígidos, tornando-os menos frequentes.
...
<!-- ========================================================== -->
```{r reglogit-freq-fit, echo=FALSE}
...
#### Avaliação do ajuste e propriedades estatísticas do modelo

```{r, echo=FALSE}
#| label: tbl-logit-freq-1
#| tbl-cap: "Estimativas do modelo de regressão logística: coeficientes, odds ratios e intervalos de confiança de 95%."
...
tabela_logit <- tidy(
  ...
  mutate(
    ...
    `IC 95% -` = round(exp(conf.low), 3),
    `IC 95% +` = round(exp(conf.high), 3)
  ) |>
  select(
    term,
    Beta,
...
kable(
  tabela_logit_fmt,
  caption = "Estimativas do modelo de regressão logística.",
  booktabs = TRUE,
  digits = 2,
  format = "latex"
)
...
O modelo de regressão logística foi estimado por máxima verossimilhança, tendo como variável resposta o status do empréstimo e como variáveis explicativas os atributos socioeconômicos e financeiros do solicitante. A @tbl-logit-freq-1 apresenta as estimativas dos coeficientes, erros-padrão, estatísticas z e respectivos valores-p.

O modelo apresentou redução substancial da deviance, passando de 
`r round(null_dev, 1)` no modelo nulo para `r round(res_dev, 1)` no modelo ajustado, com critério de informação de Akaike (AIC) igual a `r round(aic_val, 1)`, indicando que a inclusão das variáveis explicativas contribui significativamente para a explicação da variável resposta. Adicionalmente, foi calculado o Critério de Informação Bayesiano (BIC), cujo valor foi de `r round(bic_val, 1)`. 
...
##### Avaliação do classificador a partir das probabilidades estimadas
```{r, include=FALSE}
...
##### Avaliação do desempenho classificatório sob limiar de decisão
```{r, include=FALSE}
...
```{r, echo=FALSE}
#| label: tbl-confusao-treino-freq
#| tbl-cap: "Matriz de confusão do modelo de regressão logística (conjunto de treino)"
#| echo: false
#| warning: false
...
kable(conf_treino$table)
```
```{r, echo=FALSE}
#| label: tbl-confusao-teste-freq
#| tbl-cap: "Matriz de confusão do modelo de regressão logística (conjunto de teste)"
#| echo: false
#| warning: false
...
```
Após a definição do limiar de decisão, as previsões probabilísticas foram convertidas em classes, permitindo a construção das matrizes de confusão para os conjuntos de treino e teste (Tabelas @tbl-confusao-treino-freq e @tbl-confusao-teste-freq).

A partir dessas matrizes, foram calculadas métricas clássicas de desempenho, incluindo acurácia, precisão, sensibilidade, especificidade e F1-score, cujos valores estão sumarizados na @metricas-freq.
