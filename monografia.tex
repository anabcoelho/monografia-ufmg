...
Variáveis discretas vs contínuas Distribuições como forma de modelar
incerteza aprovação de crédito → Bernoulli/Binomial covariáveis
contínuas → Normal, log-normal (sem entrar em detalhes)

\subsubsection{{[}TO DO{]} Conceito de incerteza
estatística}\label{to-do-conceito-de-incerteza-estatuxedstica}

mostrar por que precisamos de inferência Incerteza decorrente de:
variabilidade natural amostragem Diferença entre: erro aleatório
incerteza paramétrica Por que modelos probabilísticos são necessários em
problemas reais

\subsubsection{{[}TO DO{]} Inferência como problema de
estimação}\label{to-do-inferuxeancia-como-problema-de-estimauxe7uxe3o}

Inferência como processo de aprender sobre parâmetros desconhecidos a
partir dos dados Estimação pontual vs intervalar Ideia geral de: Máxima
Verossimilhança (sem fórmulas) Distribuição a posteriori (apenas
conceitual)
...
realizadores aleatórios Probabilidade como grau de crença (bayesiana)
Probabilidade como medida de incerteza subjetiva/coerente Parâmetros
como variáveis aleatórias Papel da distribuição a priori
...
(heatmap), no qual a intensidade e o sinal das correlações são indicados
por uma escala de cores contínua. Essa etapa teve caráter exploratório e
diagnóstico, servindo como subsídio para a avaliação de possíveis
problemas de multicolinearidade na etapa de modelagem, especialmente no
que se refere à estabilidade e interpretação dos coeficientes da
regressão logística.
